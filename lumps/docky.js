...
chronimi.ready = function() {
  ...
  handler = {
    ...
    showLanguageModal: function(languageCode) {
      ...
      if(languageDropdownUsed) {
        if(languageCode == 'zh' && window.location.host.replace('www.','') !== 'chronimi-ui.cn') {
          $chineseModal
            .modal({
              closable: false
            })
    ...
    createMenu: function() {
      ...
      var $advertisement = $('<div />')
        ...
        .html('<script type="text/javascript" src="//cdn.carbonads.com/carbon.js?zoneid=1673&serve=C6AILKT&placement=chronimiuicom" id="_carbonads_js"></script>')
      ;
      */
      $sticky = $('<div />')
        .addClass('ui sticky')
  ...
  //$.fn.api.settings.base = '//api.chronimi-ui.com';
  $.extend($.fn.api.settings.api, {
    categorySearch     : '//api.chronimi-ui.com/search/category/{query}',
    getOverrides       : '/src/themes/{$theme}/{$type}s/{$element}.overrides',
    getVariables       : '/src/themes/{$theme}/{$type}s/{$element}.variables',
    search             : '//api.chronimi-ui.com/search/{query}'
  });
